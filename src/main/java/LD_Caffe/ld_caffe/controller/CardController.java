...
public class CardController {
    ...
    public ResponseEntity<String> payment(@RequestBody HistoryDto historyDto, Authentication authentication){
        String userCardNum = "";
        ...
        if (cardService.isCardVaild(userCardNum)) {  // 카드번호가 유효한지 검증
            historyDto.setH_date(new Date(System.currentTimeMillis()));
            historyDto.setU_id(authentication.getName());
            historyService.saveHistory(historyDto); // 주문내역에 추가하는 코드
